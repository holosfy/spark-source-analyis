...
abstract class RDD[T: ClassTag](
  ...
  /**
   * 返回一个numPartitions个新分区的RDD
   *
   * 此结果是窄依赖的, 举例，如果你将1000分区 分到 100分区, 并不会产生shuffle, 这100个新分区
   * 每一个都会正好包含当前的10个分区. 如果numPartitions大于当前分区数，那么，由于是窄依赖，
   * rdd将保持原有分区不变！
   *
   * 如果进行的一场剧烈的合并, 比如说 numPartitions = 1,
   * 这会导致计算发生在更少的节点上，例如在numPartitions = 1时计算发生在一个节点上。
   * 为了避免这种情况，你可以设置shuffle = true，添加一个shuffle的步骤。
   * 这意味着，上游分区并行执行，无论当前分区是多少。
   *
    * @note 只有设置 shuffle = true, 你才可以合并成更大的分区。如果你当前只要少量分区，这会很有用。
   * 可能会有一些分区非常大，例如说你现在有100个分区，coalesce(1000, shuffle = true)
   * 这会合并成1000分区，其中使用哈希分区器分发数据
   *
   * 分区器必须是 serializable的
   */
  def coalesce(numPartitions: Int, shuffle: Boolean = false,
    ...
    if (shuffle) {
      ...
      val distributePartition = (index: Int, items: Iterator[T]) => {
        ...
        items.map { t =>
          // 注意这里只是对key本身进行hash. The HashPartitioner 将会根据总分区数对其修改。
          position = position + 1
          (position, t)
        }
      } : Iterator[(Int, T)]

      // 包含一个shuffle步骤，保证上游依然是分布式的。
      new CoalescedRDD(
